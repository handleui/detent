...
// ActPathResolver is a function that returns the path to the act binary.
// This allows CLI-specific binary discovery to be injected without core depending on it.
// If the resolver returns an error or empty string, "act" from PATH is used as fallback.
type ActPathResolver func() (string, error)

// DefaultActPathResolver is the default resolver that uses "act" from PATH.
// Can be overridden by CLI to inject bundled binary discovery.
var DefaultActPathResolver ActPathResolver = func() (string, error) {
	return "act", nil
}
...
type RunConfig struct {
	...
	Verbose      bool
	WorkDir      string
	ActBinary    string            // Explicit path to act binary (overrides resolver)
...
func Run(ctx context.Context, cfg *RunConfig) (*RunResult, error) {
	...
	if actBinary == "" {
		// Use resolver to find act binary (allows CLI to inject bundled binary discovery)
		if resolvedPath, resolveErr := DefaultActPathResolver(); resolveErr == nil && resolvedPath != "" {
			// Check if file exists when a specific path is returned
			if resolvedPath != "act" {
				if info, statErr := os.Stat(resolvedPath); statErr == nil && !info.IsDir() {
					actBinary = resolvedPath
				} else {
					actBinary = "act"
				}
			} else {
