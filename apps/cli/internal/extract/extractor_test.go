...
func TestSanitizePatternForSentry_Length(t *testing.T) {
	...
	result := sanitizePatternForSentry(longPattern)

	if len(result) > maxUnknownPatternLineLength+10 {
		t.Errorf("sanitizePatternForSentry() result length = %d, want <= %d", len(result), maxUnknownPatternLineLength+10)
	}

	if !strings.HasSuffix(result, "...") {
		t.Error("sanitizePatternForSentry() should add '...' to truncated pattern")
	}
}
...
func TestSanitizePatternForSentry_APIKeys(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := sanitizePatternForSentry(tt.input)

			if tt.wantFree {
				// Result should not contain the original sensitive data
				if result == tt.input {
					t.Errorf("sanitizePatternForSentry() did not redact sensitive data in %q", tt.input)
				}
				// Should contain a redaction marker
				if !strings.Contains(result, "REDACTED") && !strings.Contains(result, "[") {
					t.Errorf("sanitizePatternForSentry() = %q, should contain redaction marker", result)
				}
			}
		})
	}
...
func TestSanitizePatternForSentry_FilePaths(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result != tt.want {
				t.Errorf("sanitizePatternForSentry() = %q, want %q", result, tt.want)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(result, tt.shouldMatch) {
				t.Errorf("sanitizePatternForSentry() = %q, should contain %q", result, tt.shouldMatch)
			}

			if strings.Contains(result, tt.shouldNotMatch) {
				t.Errorf("sanitizePatternForSentry() = %q, should not contain original token", result)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(result, "user:pass") {
				t.Errorf("sanitizePatternForSentry() = %q, should redact credentials", result)
			}

			// Should contain CONNECTION_STRING marker
			if !strings.Contains(result, "[CONNECTION_STRING]") {
				t.Errorf("sanitizePatternForSentry() = %q, should contain [CONNECTION_STRING]", result)
			}
		})
	}
}
...
func TestSanitizePatternForSentry_URLWithCredentials(t *testing.T) {
	...
	result := sanitizePatternForSentry(input)

	// Should not contain the original credentials
	if strings.Contains(result, "username:password") {
		t.Errorf("sanitizePatternForSentry() = %q, should redact URL credentials", result)
	}
	...
	if result == input {
		t.Errorf("sanitizePatternForSentry() did not modify input with credentials")
	}
}
...
func TestSanitizePatternForSentry_IPAddress(t *testing.T) {
	...
	if strings.Contains(result, "192.168.1.100") {
		t.Errorf("sanitizePatternForSentry() = %q, should redact IP address", result)
	}

	if !strings.Contains(result, "[IP_ADDR]") {
		t.Errorf("sanitizePatternForSentry() = %q, should contain [IP_ADDR]", result)
	}
}
...
func TestSanitizePatternForSentry_HexStrings(t *testing.T) {
	...
	if strings.Contains(result, "abcdef1234567890abcdef1234567890abcdef12") {
		t.Errorf("sanitizePatternForSentry() = %q, should redact long hex strings", result)
	}
}
...
func TestSanitizePatternForSentry_Base64(t *testing.T) {
	...
	if strings.Contains(result, "YWJjZGVmZ2hpamtsbW5vcHFyc3R1dnd4eXoxMjM0NTY3ODkwYWJjZGVm") {
		t.Errorf("sanitizePatternForSentry() = %q, should redact long base64 strings", result)
	}
}
...
func BenchmarkSanitizePatternForSentry(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		sanitizePatternForSentry(pattern)
	}
}
